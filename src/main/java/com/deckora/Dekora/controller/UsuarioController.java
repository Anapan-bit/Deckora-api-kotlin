...
public class UsuarioController {
    ...
    public ResponseEntity<EntityModel<Usuario>> createUsuario(
            @Parameter(description = "Detalles del usuario", required = true) @RequestBody Usuario usuario) {
        Usuario newUsuario = usuarioService.save(usuario);
        return ResponseEntity
                .created(linkTo(methodOn(UsuarioController.class).getUsuarioById(Long.valueOf(newUsuario.getId())))
                        .toUri())
    ...
    public ResponseEntity<EntityModel<Usuario>> patchUsuario(
            @Parameter(description = "Id del usuario", required = true, example = "1") @PathVariable Long id,
            @Parameter(description = "Campo a modificar", required = true) @RequestBody Usuario parcialUsuario) {
